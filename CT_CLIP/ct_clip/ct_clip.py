...
class CTCLIP(nn.Module):
    ...
    def load_checkpoint(self, local_dir="models/ct_clip",file_name="CT-CLIP_v2.pt"):
        working_dir= os.getcwd()
        local_path = os.path.join(working_dir,local_dir, file_name)
        if not  os.path.isfile(local_path):  
            clip_paths={"CT-CLIP_v2.pt":'https://huggingface.co/datasets/ibrahimhamamci/CT-RATE/resolve/main/models/CT-CLIP-Related/CT-CLIP_v2.pt',
                      "CT_VocabFine_v2.pt":"https://huggingface.co/datasets/ibrahimhamamci/CT-RATE/resolve/main/models/CT-CLIP-Related/CT_VocabFine_v2.pt",
                     "CT_LiPro_v2.pt":"huggingface.co/datasets/ibrahimhamamci/CT-RATE/resolve/main/models/CT-CLIP-Related/CT_LiPro_v2.pt"    
                      }
            # clip_vocab_fine=
            # clip_class_fine=https://huggingface.co/datasets/ibrahimhamamci/CT-RATE/resolve/main/models/CT-CLIP-Related/CT_LiPro_v2.pt
            utils.download_from_hub(clip_paths[file_name],local_path)
        self.load(local_path)
    def getImageEmbedding(self,scan):
       
        enc_scan = self.visual_transformer(scan,return_encoded_tokens=True) 
        enc_scan = torch.mean(enc_scan, dim=1) 
        enc_scan = enc_scan.view(enc_scan.shape[0], -1)
        image_latents = self.to_visual_latent(enc_scan)
        image_latents_normlaized= F.normalize(image_latents, dim=-1)
        return image_latents_normlaized
    ...
    def getTextEmbedding(self,text):
        ...
        text_embeddings = self.text_transformer(input_ids, attention_mask=attention_mask)
        enc_text = text_embeddings[0]
        #take the cls token of the sentance  (classifcation token)
        enc_text = enc_text[:,0,:]
        text_latents = self.to_text_latent(enc_text)
        text_latent_normlaized=F.normalize(text_latents, dim=-1)  # L2 normalize the output
        return text_latent_normlaized
